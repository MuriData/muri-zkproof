...
func ExportProofFixture(keysDir string) ([]byte, error) {
	fmt.Println("Compiling archive_poi circuit...")
	ccs, err := setup.CompileCircuit(&ArchivePoICircuit{})
	if err != nil {
		return nil, fmt.Errorf("compile circuit: %w", err)
	}

	fmt.Println("Loading keys...")
	pk, vk, err := setup.LoadKeys(keysDir, "archive_poi")
	if err != nil {
		return nil, fmt.Errorf("load keys: %w", err)
	}

	// Create a deterministic test archive: 3 files of 1, 2, 3 chunks
	files := [][]byte{
		make([]byte, FileSize),
	...
	result, err := PrepareWitness(secretKey, randomness, origTree, replicaTree, encChunks)
	if err != nil {
	...
	fmt.Println("[commitment, randomness, publicKey, archiveOriginalRoot, archiveReplicaRoot]")
...
// getAllChunks splits all files into chunks in order.
...
// frElementsToChunkBytes converts a flat fr.Element array back to chunk byte arrays.
